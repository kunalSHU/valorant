...
const verifyAccount = async (emailAddress, password) => {
  ...
  try {
    const {
      ...
      'SELECT password_salt FROM accounts_tbl WHERE email_address=$1',
      [emailAddress]
    );

    if (selectedAccountRows[0] == undefined) {
      throw  new Error(`Account Does not exist`)
    }

    const passwordSalt = selectedAccountRows[0].password_salt;
    const hashedPassword = bcrypt.hashSync(password, passwordSalt);
    ...
    try {
      ...
      const { rows: selectedJwtTokenRows } = await db.query(
        'SELECT jwt_token FROM tokens_tbl WHERE account_id=$1',
        [accountId]
      );
