...
const root = {
  ...
  allAppointments: () => knex('bookings_info.appointments_info_basic_tbl').select('*'),
  appointmentById: ({ id }) =>
    knex('bookings_info.appointments_info_basic_tbl').select('*').where({ appointmentid: id }),
  appointmentByUserId: ({ userId }) =>
    knex('bookings_info.appointments_info_basic_tbl').select('*').where({ userid: userId }),
  prescribedMedicationByAppointmentId: ({ appointmentId }) =>
    knex('bookings_info.prescribed_medications_tbl').select('*').where({ appointmentid: appointmentId }),
  userAllergiesByUserId: ({ userId }) =>
    knex('bookings_info.user_allergy_tbl')
      .join(
        'bookings_info.allergy_tbl',
        'bookings_info.user_allergy_tbl.allergyid',
        '=',
        'bookings_info.allergy_tbl.allergyid'
      )
      .select(
        'bookings_info.user_allergy_tbl.userid',
        'bookings_info.user_allergy_tbl.allergyid',
        'allergyname',
        'otherfacts'
      )
